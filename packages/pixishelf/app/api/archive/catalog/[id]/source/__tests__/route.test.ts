...
vi.mock('@/lib/prisma', () => ({ prisma: { archiveUploaderCatalogItem: { findUnique: mocks.findUnique } } }))
import { GET as getSource } from '../route'

const canonicalUrl = 'https://e-hentai.org/g/123/private123/'
const invoke = (id = 'archive-id') =>
  getSource(new Request('http://localhost/api/archive/catalog/archive-id/source?url=https://evil.test'), {
...
describe('archive source redirect route', () => {
  ...
  it.each([null, { user: {} }])(
    'rejects missing or invalid sessions before reading private catalog items',
    async (session) => {
  ...
  it('redirects a signed-in request to its validated catalog item gallery, ignoring client URLs', async () => {
    const response = await invoke()
  ...
  it('rejects an invalid catalog item id before database access', async () => {
  ...
  it.each([null, { providerKey: 'e-hentai', externalId: '123', canonicalUrl: 'https://evil.test/private123/' }])(
    'does not redirect missing catalog items or unsafe locators',
    async (item) => {
