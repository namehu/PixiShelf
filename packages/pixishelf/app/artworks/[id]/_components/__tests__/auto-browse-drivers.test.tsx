...
describe('scroll driver', () => {
  ...
  it('waits for player readiness and completion, then continues once per cycle', () => {
    const { advance, getY } = setup({ animated: true })
    advance(0)
    expect(store.getState()).toMatchObject({ activeAnimationId: 1, status: 'waiting' })
    advance(10000)
    expect(getY()).toBe(0)
    act(() => store.getState().animationReady(1))
    advance(10001)
    expect(store.getState().status).toBe('running')
    advance(12300)
    expect(getY()).toBe(0)
    act(() => store.getState().finishAnimation(1))
    advance(12351)
    expect(getY()).toBeGreaterThan(0)
  ...
  it('plays the final animation before ending and replays it on the next loop', () => {
    const { advance } = setup({ animated: true, height: 800 })
  ...
  it('keeps an animation failure paused until skip and resume', () => {
    const { advance, getY } = setup({ animated: true })
    advance(0)
  ...
  it('does not let an adjacent unloaded preview delay the current animation clock', () => {
    ...
    act(() => store.getState().finishAnimation(1))
    advance(5001)
    expect(store.getState()).toMatchObject({ activeAnimationId: null, completedAnimationIds: [1] })
    advance(5002)
    expect(store.getState().status).toBe('waiting')
  })
