...
import { cancelPixivTagEnrichment } from '../pixiv-tag-enrichment-service'

const testDatabaseUrl = Reflect.get(process.env, 'PIXISHELF_TEST_DATABASE_URL') as string | undefined
const describePostgres = testDatabaseUrl ? describe : describe.skip
const suitePrefix = `pixiv-tag-cancel-${randomUUID()}`
...
describePostgres('Pixiv tag enrichment PostgreSQL cancellation', () => {
  beforeAll(async () => {
  ...
  it('transitions the whole batch by child state and keeps repeated cancellation idempotent', async () => {
    ...
    await expect(cancelPixivTagEnrichment(rootId, database)).resolves.toMatchObject({
      batchId: rootId,
      affectedCount: 4
    })
    ...
    await expect(cancelPixivTagEnrichment(rootId, database)).resolves.toMatchObject({ affectedCount: 0 })
  ...
  it('turns a child claimed while cancellation waits for its row lock into cancelling', async () => {
    ...
    const cancellation = cancelPixivTagEnrichment(rootId, database)
...
async function seedChildren(rootId: string, statuses: SeedStatus[]) {
  ...
  const children = statuses.map((status, index) => ({
    ...
    requestedByUserId,
    parentJobId: rootId,
    payload: { mode: 'TAG', tagId: index + 1, expectedName: `tag-${index + 1}`, force: false },
