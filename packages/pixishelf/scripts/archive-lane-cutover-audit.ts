import {
  createPrismaArchiveLaneCutoverAuditReader,
...
async function main(): Promise<0 | 2> {
  ...
  try {
    const report = await runCutoverAudit(createPrismaArchiveLaneCutoverAuditReader(prisma), { sampleLimit })
...
main()
  ...
  .catch((error) => {
    process.stderr.write(
      `Archive lane cutover audit failed: ${error instanceof Error ? error.message : String(error)}\n`
    )
    process.exitCode = 1
  })
