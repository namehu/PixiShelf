...
export function STable<T extends Record<string, any>>({
              ...
              <div key={key} className="relative">
                {rowSelection && (
                  <div className="absolute top-4 right-4 z-10">
                    <Checkbox checked={isSelected} onCheckedChange={(checked) => handleSelectRow(!!checked, record)} />
                  </div>
                )}
  ...
  return (
    <div className={cn('w-full space-y-4', className)}>
      {/* 搜索区域 */}
      <SSearch columns={columns} onSearch={handleSearch} onReset={handleReset} initialValues={defaultSearchParams} />
      ...
      <div className="bg-white rounded-lg border border-neutral-200 overflow-hidden">
        ...
        {isLoading ? (
          ...
          <>
                            ...
                            {col.sorter && (
                              <ArrowUpDown
                                className={cn(
                                  ...
                                  sortParams[col.key || (col.dataIndex as string)]
                                    ? 'text-blue-600'
                                    : 'text-neutral-400'
                                )}
            ...
            {/* Mobile View */}
            <div className="md:hidden p-4">{renderMobileList()}</div>
          </>
        )}
      </div>
