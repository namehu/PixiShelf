 ...
 * @returns 收集结果
 */
export async function collectMediaFiles(directoryPath: string, artworkId: string): Promise<CollectionResult> {
  try {
    // 读取目录内容
    ...
    for (const entry of files) {
      ...
      // 检查是否是目标作品的媒体文件
      const mediaInfo = parseMediaFilename(filename, artworkId, extension)
