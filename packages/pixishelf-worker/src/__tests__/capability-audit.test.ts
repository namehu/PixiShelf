...
describe('production Worker capability audit', () => {
  it('accepts exactly one fresh READY Worker with 25 job types and both versioned job families', async () => {
  ...
  it('rejects an incomplete inventory when SCAN only advertises v1', async () => {
    ...
    ).rejects.toThrow('25-job/28-version dual-lane release')
  })
  ...
  it('reports job type count without treating SCAN versions as separate capabilities', async () => {
    ...
    expect(writeOutput).toHaveBeenCalledWith(
      'Worker capability audit passed: 1 READY Worker, 25 job types / 28 versions (SCAN v1/v2/v3, ARCHIVE_IMPORT v1/v2)'
    )
  })
