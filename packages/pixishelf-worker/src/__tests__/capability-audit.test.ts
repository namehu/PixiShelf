...
describe('production Worker capability audit', () => {
  it('accepts exactly one fresh READY Worker with 33 job types and three versioned job families', async () => {
    ...
    ).resolves.toEqual({ readyWorkers: 1, capabilities: 33 })
  ...
  it.each(['SCAN', 'ARCHIVE_SEARCH_SCAN'])(
    ...
    async (jobType) => {
      ...
      ).rejects.toThrow('33-job/38-version dual-lane release')
    }
  )
  ...
  it('reports job type count without treating SCAN versions as separate capabilities', async () => {
    ...
    expect(writeOutput).toHaveBeenCalledWith(
      'Worker capability audit passed: 1 READY Worker, 33 job types / 38 versions (SCAN v1/v2/v3, ARCHIVE_IMPORT v1/v2, ARCHIVE_SEARCH_SCAN v1/v2/v3)'
    )
  })
