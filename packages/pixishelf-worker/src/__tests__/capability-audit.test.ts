...
describe('production Worker capability audit', () => {
  it('accepts exactly one fresh READY Worker with 32 job types and three versioned job families', async () => {
    ...
    ).resolves.toEqual({ readyWorkers: 1, capabilities: 32 })
  ...
  it.each(['SCAN', 'ARCHIVE_SEARCH_SCAN'])(
    ...
    async (jobType) => {
      ...
      ).rejects.toThrow('32-job/37-version dual-lane release')
    }
  )
  ...
  it('reports job type count without treating SCAN versions as separate capabilities', async () => {
    ...
    expect(writeOutput).toHaveBeenCalledWith(
      'Worker capability audit passed: 1 READY Worker, 32 job types / 37 versions (SCAN v1/v2/v3, ARCHIVE_IMPORT v1/v2, ARCHIVE_SEARCH_SCAN v1/v2/v3)'
    )
  })
