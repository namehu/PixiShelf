...
export async function auditProductionWorkerCapabilities(
  ...
  try {
    ...
    throw new CapabilityAuditError(
      'online READY Worker capability inventory does not match the 33-job/38-version dual-lane release'
    )
  }
...
export async function runCapabilityAudit(
  ...
  writeOutput(
    `Worker capability audit passed: ${result.readyWorkers} READY Worker, ${result.capabilities} job types / 38 versions (SCAN v1/v2/v3, ARCHIVE_IMPORT v1/v2, ARCHIVE_SEARCH_SCAN v1/v2/v3)`
  )
  return 0
}
