...
describe('database package', () => {
  it('accepts the complete background queue schema contract', async () => {
    const client = createQueryClient([
      ...
      [{ migrationName: '20260814110000_add_single_dispatcher_execution_fence' }],
      [expectedIndex]
    ])
  ...
  it('reports missing required objects without exposing connection details', async () => {
    ...
    await expect(assertBackgroundQueueSchema(client)).rejects.toThrow(
      'Background queue schema is not ready: missing system_jobs.definitionVersion, derived_media_gc_entries, job_resource_leases, system_job_events, worker_instances, migration:20260814110000_add_single_dispatcher_execution_fence, index:system_jobs_single_executing_job_idx'
    )
  })
