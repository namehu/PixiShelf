...
export class SafeHttpClient {
  ...
  private async requestFollowingRedirects(
    ...
    if (response.status >= 300 && response.status < 400 && response.headers.location) {
      ...
      if (nextMethod) nextOptions.method = nextMethod
      else delete nextOptions.method
      if (nextMethod === 'GET') delete nextOptions.body
      return this.requestFollowingRedirects(redirected, nextOptions, redirectCount + 1)
    }
...
function sendRequest(
  ...
  return new Promise((resolve, reject) => {
    ...
    if (options.signal?.aborted) {
      reject(classifyNetworkError(requestCancellationError(options.signal), connectionStage, targetHost))
      return
    }
    ...
    const onAbort = () => request.destroy(requestCancellationError(options.signal))
...
function requestCancellationError(signal?: AbortSignal) {
  return new ArchiveError('CANCELLED', '请求已取消', {
    recoverable: true,
