...
type PosterOperationContext = Omit<PosterContext, 'payload'>

export type VideoPosterBatchItemResult =
  | { kind: 'generated'; imageId: number; posterPath: string }
  | { kind: 'skipped'; imageId: number; message: string }
  | { kind: 'failed'; imageId: number; errorCode: JobErrorCode; message: string }
...
export async function executeVideoPoster(
  ...
  return runVideoPoster(context, dependencies, context.payload, 'standalone')
}

export async function generatePendingVideoPoster(
  context: PosterOperationContext,
  dependencies: { database: VideoMediaDatabase; config: VideoMediaRuntimeConfig; now?: () => Date },
  payload: VideoPosterPayload
): Promise<VideoPosterBatchItemResult> {
  return runVideoPoster(context, dependencies, payload, 'batch')
}

async function runVideoPoster(
  context: PosterOperationContext,
  dependencies: { database: VideoMediaDatabase; config: VideoMediaRuntimeConfig; now?: () => Date },
  payload: VideoPosterPayload,
  mode: 'standalone'
...
async function runVideoPoster(
...
): Promise<JobExecutionOutcome | VideoPosterBatchItemResult> {
  ...
  try {
    ...
    if (normalizePath(image.path) !== normalizePath(payload.relativePath)) {
    ...
    if (
      image.videoMetadata?.manualPosterTimestamp !== null &&
      image.videoMetadata?.manualPosterTimestamp !== undefined
    ) {
      return skippedPosterOutcome(mode, image.id, '视频已有人工封面')
    }
    ...
    if (image.videoMetadata?.posterStatus === 'COMPLETED' && !completedPosterMissing) {
      return skippedPosterOutcome(mode, image.id, '视频封面已存在')
    }
    ...
    claimed = await context.mutateInTransaction<VideoMediaTransaction & QueueSqlExecutor, boolean>(
      async (transaction) => {
    ...
    if (!claimed) return skippedPosterOutcome(mode, image.id, '视频封面状态已变化')
    ...
    if (context.signal.aborted) throw context.signal.reason ?? new Error('Video poster generation was interrupted')
    if (mode === 'batch') {
      finalizationStarted = true
      const published = await context.mutateInTransaction<VideoMediaTransaction & QueueSqlExecutor, boolean>(
        (transaction) =>
          publishPosterAttempt(transaction, {
            imageId: image.id,
            temporaryPath: temporaryPath!,
            outputPath: outputPath!,
            relativePosterPath: relativePosterPath!,
            relativeTemporaryPath: relativeTemporaryPath!,
      ...
      return published
        ? { kind: 'generated', imageId: image.id, posterPath: relativePosterPath }
        : { kind: 'skipped', imageId: image.id, message: '视频封面所有权已变化' }
    }
    finalizationStarted = true
    const outcome = await context.finalizeInTransaction<VideoMediaTransaction & QueueSqlExecutor>(async (scope) => {
      ...
      const published = await publishPosterAttempt(scope.transaction, {
        imageId: image.id,
        temporaryPath: temporaryPath!,
        outputPath: outputPath!,
        relativePosterPath: relativePosterPath!,
      ...
      await scope.complete({
        result: { imageId: image.id, posterPath: relativePosterPath },
        message: '视频封面生成完成'
      })
    })
    await fs.rm(temporaryPath!, { force: true }).catch(() => undefined)
    ...
    if (context.signal.aborted && mode === 'batch') {
      if (claimed) {
        await context.mutateInTransaction<VideoMediaTransaction & QueueSqlExecutor>((transaction) =>
          resetPoster(transaction, payload.imageId, 'PENDING', null)
        )
      }
      throw error
    }
    if (context.signal.aborted) {
      return context.finalizeInTransaction<VideoMediaTransaction & QueueSqlExecutor>((scope) =>
        finalizePosterControl(scope, payload.imageId, claimed, true).then(async (handled) => {
    ...
    return context.finalizeInTransaction<VideoMediaTransaction & QueueSqlExecutor>(async (scope) => {
      if (await finalizePosterControl(scope, payload.imageId, claimed)) return
      if (claimed) {
...
function skippedPosterOutcome(
  mode: 'standalone' | 'batch',
  imageId: number,
  message: string
): JobExecutionOutcome | VideoPosterBatchItemResult {
  return mode === 'batch'
    ? { kind: 'skipped', imageId, message }
    : { kind: 'skipped', reason: 'PRECONDITION_NOT_MET', message }
}

interface PosterPublicationAttempt {
  imageId: number
...
async function publishPosterAttempt(
  transaction: VideoMediaTransaction,
  attempt: PosterPublicationAttempt
): Promise<boolean> {
  await lockVideoPoster(transaction, attempt.imageId)
  const current = await transaction.mediaVideoMetadata.findUnique({
    where: { imageId: attempt.imageId },
    select: { posterStatus: true, posterPath: true, manualPosterTimestamp: true }
  })
  if (current?.posterStatus !== 'GENERATING' || current.manualPosterTimestamp !== null) return false

  const previousPath = current.posterPath
  await fs.rename(attempt.temporaryPath, attempt.outputPath)
  const updated = await transaction.mediaVideoMetadata.updateMany({
    where: { imageId: attempt.imageId, posterStatus: 'GENERATING', manualPosterTimestamp: null },
    data: {
      posterStatus: 'COMPLETED',
      posterPath: attempt.relativePosterPath,
      posterUpdatedAt: attempt.now(),
      posterError: null
    }
  })
  if (updated.count !== 1) throw new Error('Video poster ownership changed during publication')
  if (previousPath && previousPath !== attempt.relativePosterPath) {
    await transaction.derivedMediaGcEntry.upsert({
      ...
      create: {
        ...
        referenceType: 'MEDIA_VIDEO_METADATA_POSTER',
        referenceId: String(attempt.imageId),
        reason: 'POSTER_REPLACED',
        status: 'PENDING',
        notBefore: new Date(attempt.now().getTime() + GC_DELAY_MS),
        lastSystemJobId: attempt.systemJobId
      },
      update: {
        referenceType: 'MEDIA_VIDEO_METADATA_POSTER',
        referenceId: String(attempt.imageId),
  ...
  await transaction.derivedMediaGcEntry.deleteMany({
    where: {
      mediaKind: 'VIDEO_POSTER',
      OR: [
        { relativePath: attempt.relativePosterPath, reason: 'POSTER_ATTEMPT_OUTPUT' },
        { relativePath: attempt.relativeTemporaryPath, reason: 'POSTER_ATTEMPT_TEMPORARY' }
      ]
    }
  })
  return true
