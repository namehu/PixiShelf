...
export class ScannerService {
  ...
  private async parseAndCollect(metadataFile: GlobMetadataFile): Promise<ArtworkData | null> {
    ...
    try {
      ...
      if (!collectionResult.success) {
        this.logger.warn({ metadataPath, artworkId, error: collectionResult.error }, 'Failed to collect media files')
        return null
      }

      if (collectionResult.mediaFiles.length === 0) {
        this.logger.warn({ metadataPath, artworkId }, 'No media files found for artwork')
        return null
      }
  ...
  private async batchProcessArtists(artworks: ArtworkData[]): Promise<void> {
    ...
    if (uncachedUserIds.size === 0) {
      this.logger.debug('All artists in current batch are already cached, skipping artist processing')
      return
    }

    this.logger.debug({ uncachedArtists: uncachedUserIds.size }, 'Processing uncached artists in current batch')
  ...
  private async batchProcessTags(artworks: ArtworkData[]): Promise<void> {
    ...
    if (uncachedTagNames.size === 0) {
      this.logger.debug('All tags in current batch are already cached, skipping tag processing')
      return
    }

    this.logger.debug({ uncachedTags: uncachedTagNames.size }, 'Processing uncached tags in current batch')
