...
function useScanPath() {
  ...
  return {
    query: useQuery({
      queryKey: ["scanPath"],
      queryFn: async () => {
        return apiJson<{ scanPath: string | null }>(
          "/api/v1/settings/scan-path",
        );
      },
    }),
    update: useMutation({
...
function useCancelScan() {
  return useMutation({
    mutationFn: async () => {
      return apiJson<{ success: boolean; cancelled: boolean }>(
        "/api/v1/scan/cancel",
        {
          method: "POST",
        },
...
const useEventSource =
  typeof window !== "undefined" ? (url: string) => new EventSource(url) : null;
...
export default function Settings() {
  ...
  const [streamResult, setStreamResult] = React.useState<ScanResult | null>(
    null,
  );
        ...
        es.addEventListener("progress", (ev: any) => {
          try {
            ...
            addLogEntry(
              "progress",
              data,
              `进度更新: ${data.message} (${data.percentage || 0}%)`,
            );
          } catch (e) {
            console.warn("Failed to parse progress event:", e);
            addLogEntry(
              "error",
              { error: e, rawData: ev.data },
              "解析进度事件失败",
            );
          }
        });
        ...
        <div className="mb-3 flex items-center justify-between gap-3">
          <div>
            ...
            <div className="text-sm text-gray-500">
              使用 Server-Sent Events 实时显示扫描进度。支持自动重连。
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              disabled={
                scan.isPending || streaming || !scanPath.query.data?.scanPath
              }
            ...
            <ul className="grid grid-cols-2 gap-2">
              <li className="rounded border bg-gray-50 p-2">
                ...
                <span className="font-medium">
                  {streamResult.scannedDirectories}
                </span>
              </li>
              <li className="rounded border bg-gray-50 p-2">
                发现图片：
              ...
              {typeof streamResult.removedArtworks === "number" && (
                <li className="rounded border bg-gray-50 p-2">
                  删除空作品：
                  <span className="font-medium">
                    {streamResult.removedArtworks}
                  </span>
                </li>
              )}
              {streamResult.skippedDirectories &&
                streamResult.skippedDirectories.length > 0 && (
                  <li className="rounded border bg-gray-50 p-2">
                    跳过目录：
                    <span className="font-medium">
                      {streamResult.skippedDirectories.length}
                    </span>
                  </li>
                )}
            </ul>
              ...
              streamResult.skippedDirectories.length > 0 && (
                <details className="rounded border bg-orange-50 p-3">
                  <summary className="cursor-pointer text-orange-800">
                    跳过目录 {streamResult.skippedDirectories.length}{" "}
                    个（展开查看）
                  </summary>
                  <ul className="mt-2 list-disc space-y-1 pl-5 text-orange-900">
                    ...
                    {streamResult.skippedDirectories.length > 10 && (
                      <li>… 仅展示前 10 条</li>
                    )}
                  </ul>
                </details>
                ...
                <div className="space-y-1 font-mono text-xs">
                  {logs.map((log, index) => (
                    <div key={index} className="flex gap-2">
                      ...
                      <span
                        className={`shrink-0 font-medium ${getLogTypeStyle(log.type)}`}
                      >
