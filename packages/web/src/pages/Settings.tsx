    ...
    const connectSSE = () => {
      ...
      es.addEventListener('complete', (ev: any) => {
        try {
          ...
          setStreaming(false)
          addLogEntry('complete', data, `扫描完成: 新增${data.result.newArtworks}个作品，${data.result.newImages}张图片`)
          es.close()
        } catch (e) {
        ...
        {streamResult && (
          <div className="mt-3 space-y-2 text-sm">
            ...
            <ul className="grid grid-cols-2 gap-2">
              ...
              {streamResult.skippedDirectories && streamResult.skippedDirectories.length > 0 && <li className="rounded border bg-gray-50 p-2">跳过目录：<span className="font-medium">{streamResult.skippedDirectories.length}</span></li>}
            </ul>
            ...
            {streamResult.skippedDirectories && streamResult.skippedDirectories.length > 0 && (
              <details className="rounded border bg-orange-50 p-3">
                <summary className="cursor-pointer text-orange-800">跳过目录 {streamResult.skippedDirectories.length} 个（展开查看）</summary>
                <ul className="mt-2 list-disc space-y-1 pl-5 text-orange-900">
                  {streamResult.skippedDirectories.slice(0, 10).map((item, i) => (
                    <li key={i}><span className="font-mono text-xs">{item.path}</span> - {item.reason}</li>
                  ))}
