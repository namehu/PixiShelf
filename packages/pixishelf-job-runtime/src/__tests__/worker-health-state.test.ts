...
describe('worker health diagnostics', () => {
  it('redacts credentials before truncating a persisted error summary', () => {
    const message = errorMessage(
      new Error(
        'postgresql://worker:database-secret@postgres/pixishelf https://api-user:api-secret@example.test Bearer bearer-secret password=plain-secret token:"token-secret"'
      )
    )
