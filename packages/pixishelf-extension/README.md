...
PixiShelf 下载助手是一个使用 WXT、React 和 TypeScript 开发的浏览器扩展。扩展运行在 Pixiv 页面内，批量采集标签和作品元数据，在浏览器本地保存任务进度，并将结果导出为 SQL 或标签封面图片。

扩展不会连接 PixiShelf Web/API，也不会自动修改 PixiShelf 数据库。生成的 SQL 是需要人工检查和执行的辅助产物。

## 实际功能

扩展在 `https://www.pixiv.net/*` 页面右上角注入一个 `P` 按钮。点击后打开可拖动、折叠的浮动面板，面板包含“标签”“作品”和“设置”三个页签。面板位置、显示状态、当前页签和下载设置会保存在浏览器本地。
...
- 标签、作品任务及日志存放在浏览器 IndexedDB 数据库 `PixiShelfDB` 中，刷新 Pixiv 页面后仍然保留。
...
- 标签和作品任务可以暂停，正在处理的项目会恢复为 `pending`。
