...
/**
 * Chrome扩展存储管理器
 * 使用Chrome Storage API替代localforage
 */
class StorageManager {
  ...
  async getTagList(): Promise<string[]> {
    try {
      const result = await chrome.storage.local.get(this.IDS_STORAGE_KEY);
      return result[this.IDS_STORAGE_KEY] || [];
    } catch (error) {
  ...
  async clearAll(): Promise<void> {
    try {
      await chrome.storage.local.remove([this.STORAGE_KEY, this.IDS_STORAGE_KEY]);
    } catch (error) {
  ...
  async getTaskStats(): Promise<TaskStats> {
    try {
      const [allTags, progress] = await Promise.all([
        this.getTagList(),
        this.getProgress()
      ]);
