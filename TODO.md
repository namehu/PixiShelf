...
清理前，`packages/pixishelf/lib/image-loader.js` 曾保留通过 Thumbor 实时截帧的兼容兜底；当前已改为
静态占位和明确错误日志，不再存在请求时视频截帧链路。

### 后续清理事项

- [x] 再次确认所有封面和缩略图入口都使用 `MediaThumbnail`，不存在把原始视频路径传给 `next/image` 的场景。
- [x] 移除 `image-loader.js` 中的 Thumbor 地址和视频实时截帧分支，并为误传视频路径保留明确的占位或错误处理。
- [x] 移除 `NEXT_PUBLIC_THUMBOR_VIDEO_URL`、`THUMBOR_HOST_PORT` 及构建时环境变量替换逻辑。
- [x] 从开发和部署 Docker Compose 中移除 Thumbor 服务、端口、卷挂载及相关路由。
- [x] 删除 `build/thumbor` 镜像构建配置和 Thumbor 镜像发布工作流。
- [x] 更新 `README.md`、`DEPLOYMENT.md`、架构文档和代理启动说明中的 Thumbor 内容。
- [x] 停止并删除现有 `thumbor_video` 容器。
- [x] 使用聚焦单测和静态配置检查回归普通图片、静态视频封面、封面缺失占位、视频播放以及 GIF/WebP 处理链路。

### 注意

本机容器已经删除但镜像仍保留；生产环境应在新版本媒体链路验证通过后，再删除旧容器和外部
Traefik `/_video` 路由。回滚旧版本时必须使用对应归档的 Compose 和路由配置。
