...
- `worker.Dockerfile`：通用后台 Worker 镜像，包含数据库客户端、任务契约、运行时和当前全部
  25 个 job type；`SCAN` 支持 v1/v2/v3，其余 24 类只支持 v1，共 27 个 type/version 组合。
...
当前通用 Registry 已锁定 25 个 job type、27 个 type/version 组合，并校验 job type、definition version 和
lane；任务清单中包括 `SCAN`、`LOCAL_DIRECTORY_IMPORT`、`MIGRATION`、`PENDING_REPLACE` 四类高风险任务，
`SCAN` 支持 v1/v2/v3，其余 24 类只支持 v1。新部署仍须先以
