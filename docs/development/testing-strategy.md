...
健康检查证明进程和两个 lane 的预检状态，capability audit 精确证明 30 个 job type、34 个 type/version 组合
（`SCAN` v1/v2/v3、`ARCHIVE_IMPORT` 与 `ARCHIVE_SEARCH_SCAN` v1/v2、其余 v1）的 type/version/lane 已注册；二者都不能代替领域功能测试。
...
Worker 测试和 capability 门禁包含双 lane contract，以及 30 个 job type、34 个 type/version 组合（`SCAN`
v1/v2/v3、`ARCHIVE_IMPORT` 与 `ARCHIVE_SEARCH_SCAN` v1/v2、其余 v1）的精确 inventory；CI 的空库 migration 仍不能替代生产数据副本或非空历史 fixture 的直切
