...
结果读取跨该来源所有仍在保留期内的已完成运行汇总，以 Provider/GID 去重并保留最新一次分类，排除全局已忽略画廊，再按远端发布时间通过服务端游标分页。前端使用虚拟列表，只渲染可视区域并在接近底部时自动加载下一页；活动扫描完成后自动刷新结果流。
...
`NEW`、`POSSIBLE_UPDATE` 和 `REPLACEMENT` 可以单项或最多 100 条批量加入收件箱或忽略；`ACTIVE`、`ARCHIVED` 和已经关联 Intake Item 的结果不可再处置。入箱服务会再次检查全局忽略表，避免旧页面绕过恢复步骤提交已忽略画廊。

忽略与加入收件箱按排序后的 Provider/GID 获取同一组事务级 advisory lock，并在锁内重新读取忽略和入箱状态。新入箱尝试的检查、Intake 创建及 Scan Item 关联处于同一事务；因此并发处置最多一方成功，批量中任一项冲突会整体回滚。同一个 `submissionAttemptId` 的已提交请求仍按原结果幂等重放。

结果区使用固定的“发现 / 已忽略”范围切换，不按扫描运行创建动态 tab。两个范围都支持虚拟无限滚动，并共享“纯列表 / 首图”显示偏好：
...
全局忽略迭代只增加 `archive_uploader_ignored_items` 表和 App 侧读写，不修改 Job type、payload、lane 或 Worker capability。该 migration 之后只需发布新 App；现有 Worker 可继续运行。回滚 App 时保留忽略表，避免丢失用户决定。
