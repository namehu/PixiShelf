...
输入使用 response.body reader，媒体不调用 arrayBuffer/blob。压缩数据从 64 KiB 容量起步、按倍数扩容且严格受文件上限约束；扩容前释放借用输入地址的 demux 和迭代器，随后从已接收前缀重建。Worker 通过可转移缓冲给主线程，最多预解码两帧并回收输出缓冲；暂停/队列满时停止主动读取与解码，允许一次在途读取完成。

| 预算           | 移动    | 桌面    |
| -------------- | ------- | ------- |
| 压缩输入       | 128 MiB | 256 MiB |
| 画布像素       | 400 万  | 800 万  |
| WASM 堆        | 384 MiB | 768 MiB |
| 引擎受管总预算 | 432 MiB | 864 MiB |
| 预解码队列     | 2 帧    | 2 帧    |

窄屏档由 `window.innerWidth < 768` 选择，并非手机硬件或真实可用内存检测。扩容采用 `realloc`，搬迁时旧 64/128 MiB 与新 128/256 MiB 块可能短暂共存，即分别约 192/384 MiB；另有两张合成画布（最多约 32/64 MB）、libwebp 工作区及 64 KiB 拷贝缓冲。因此运行与编译的 WASM 堆上限分别为 384/768 MiB，桌面 256 MiB 输入能落在模块允许的 768 MiB 最大页数内。受管预算再为两帧转移缓冲和显示画布预留至多约 48/96 MB，设为 432/864 MiB。预算不等于浏览器进程总内存，浏览器可能更早拒绝分配；真实头部尺寸与字节数为准。实例退出释放，不把 RGBA 全帧或媒体写入持久化缓存。

WASM/Worker/流能力不可用、资源加载或初始化失败、文件/像素/内存超限或 ICC/EXIF 元数据触发兼容回退，每次尝试最多一次。简短提示显示具体原因及「完整加载后开始，暂停后重播」，4 秒后隐藏，且不挡点击；切图或卸载清理计时。原因是固定类别，不包含请求路径。回退播放器不受新引擎预算保证。鉴权、网络、文件损坏走错误暂停；已经显示动画后内部错误不能自动突然从头播放。
...
### 2026-09-24 文件上限与回退反馈验证

压缩输入扩容、借用迭代器释放和循环重建由原生 ASan/UBSan 差分检查覆盖：首帧已解出且 demux/上一帧迭代器仍持有输入时，至少三次 `realloc` 实际搬迁，随后跨三轮与官方解码器逐帧比对像素；12 字节硬上限和截断输入也通过。固定 Docker 工具链重新生成 WASM 和清单，`build:native --check` 的逐字节复现通过，未手工编辑生成物。

包类型检查、10 项单元测试、5 项 prebuilt 负向校验、构建及 19 项 Edge 真实 HTTP 浏览器测试通过。浏览器直接从有效 WebP 加分块 RIFF 尾块，验证窄屏 33 MiB、宽屏 65 MiB 超过旧上限仍完整播放，129/257 MiB 分别被新硬上限拒绝；桌面测试实际创建 768 MiB 最大页数的 WASM Memory。1080p/24fps 合成样本耗时 9990.2ms，低于 11000ms 门槛。主应用相关 51 项定向用例及 typecheck 通过，包含 4 秒提示、重渲染计时、切图清理、原因展示与持久诊断属性。本次尚未执行安卓真机、真实登录反代或完整内存峰值剖析。
