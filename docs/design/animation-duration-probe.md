...
生产观察发现旧实现把全部 WebP 当时长候选，库存剩余曾显示 213,294；已读取的 60 项均为静态。旧执行器每 10 项或 5 秒主动让出 Worker，任务反复进入等待。上述“正常让行误报修复验收”只记录当时如何隐藏误报，不再描述现行调度策略。

本次将已识别动态 WebP 作为唯一候选范围，并统一用于分页、到期失败重试、库存及管理员重试；发布前在 Image 行锁下再次验证分类，防止扫描并发改类后发布旧结果。未分类及待识别项先由动画识别任务判型；同 ID 扫描发现源变化时仍失效时长结果，识别为动态后才重新进入候选。旧检查点可读取，游标到末尾时会在同次执行内回卷处理后来进入范围的低 ID 项。
