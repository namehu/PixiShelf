...
标签管理页提供人工触发的 Pixiv 补全。它可以按每批最多 200 个处理尚未检查的候选，也可以在列表中多选标签后重新查询所选项。它只处理数据库中已经存在的普通标签，不创建新标签，也不会由扫描、计划任务或页面浏览自动触发。
...
默认批量任务按标签 ID 每次只取下一批最多 200 个，并跳过已有检查状态；多选补全会强制重新查询最多 200 个所选标签，但仍只填空字段。批量任务会冻结标签 ID 和查询时的原始名称。单标签重试仍会重新验证标签名称和来源关系；标签已经删除、改名或失去 Pixiv 来源关系时不发布旧结果。
...
1. `DISCOVER` 父任务按 ID 发现下一批最多 200 个候选，为每个标签创建一个低优先级 `TAG` 子任务，然后结束；
