...
- Worker 在同一个 fenced transaction 中更新 `progress/progressData` 并插入事件；失去 lease/fence 时两者都不写。
...
`WEBP_ANIMATION_SCAN` 依次进入 `INITIALIZING`、`SCANNING`、`COMPLETED`。初始化每 500 条提交并反馈；探测使用 1–8 的内部有界 worker pool，默认由 `ANIMATION_SCAN_CONCURRENCY=4` 控制。分类结果累计 20 条或等待 2 秒即在 fenced 微批次中提交，只有领域写成功后才推进持久进度。
...
`JOB_EVENT_RETENTION_CLEANUP` 对 INFO 级 `job.progress` 保留 7 天，对阶段、警告、错误、控制和终态事件保留 90 天，每次删除最多 5,000 条。计划默认关闭；首次手动执行固定为 dry-run，核对候选数和 SSE 重连后再启用计划，计划执行才会删除。
