...
1. `DISCOVER` 默认按每页 200 个持续发现全部 `providerKey=pixiv` 且尚未检查的身份；多选模式最多重查 200 个已确认身份；管理员也可以显式开启“刷新已有资料”，按最久未检查顺序刷新全部身份或只刷新所选身份；
...
Pixiv 返回的官方默认头像占位图不视为艺术家自定义头像：补全时保持头像为空，刷新时保留当前头像，也不会因此将任务标记为 `PARTIAL`。

用户资料、头像和背景图片请求均由 Worker 使用共享 `ARCHIVE_HTTPS_PROXY` 出站策略；代理失败不降级直连，仍沿用任务重试与部分成功规则。代理只改变服务端请求路径，HTTPS 443、精确域名和每次重定向校验保持有效，配置与优先顺序见[部署基线](../operations/deployment.md#环境文件边界)。

200 是数据库发现分页大小和显式选择上限，不是默认逻辑批次上限。一个 `DISCOVER` 父任务会把当次全部候选物化为独立的低优先级 `ARTIST` 子任务；`BACKGROUND_WRITER` 始终逐个执行艺术家，不会并发下载。父任务完成发现后，执行动态继续以一个稳定的逻辑批次显示整体进度和当前处理项，直到全部子任务进入终态。关闭管理页不会停止持久队列。整批取消会先停止父任务继续派生，再在事务中批量取消所有尚未完成的子任务，已发布的资料不回滚。
