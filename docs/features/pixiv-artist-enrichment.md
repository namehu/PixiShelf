...
200 是数据库发现分页大小和显式选择上限，不是默认逻辑批次上限。一个 `DISCOVER` 父任务会把当次全部候选物化为独立的低优先级 `ARTIST` 子任务；`BACKGROUND_WRITER` 始终逐个执行艺术家，不会并发下载。关闭管理页不会停止持久队列。整批取消会先停止父任务继续派生，再在事务中批量取消所有尚未完成的子任务，已发布的资料不回滚。
