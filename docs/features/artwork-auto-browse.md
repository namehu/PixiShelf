...
| 点击暂停动图                                            | 回到海报，本轮不自动重播；预览重新计满静态停留时间，详情继续滚动 |
| 未开启或已暂停自动浏览时播放                            | 只操作动图，不开启或恢复自动浏览                                 |
| 暂停自动浏览、打开设置、手动滑动/切图/缩放/导航或切后台 | 取消自动播放及推进任务，等待用户继续；未完成动图继续时从头播放   |
...
flowchart TD
  ...
  B -->|暂停动图| C[停止动画，保持自动浏览]
  C --> D[本轮作为静态图浏览]
  B -->|暂停自动浏览或打开设置| E[停止自动播放与推进]
  E --> F[关闭设置不会自行恢复]
  F -->|点击继续| G[未完成动图从头播；已处理图片按静态浏览]
  B -->|播放或重播| H[从头播放一轮]
  H -->|完成| I[自动换图或继续滚动]
```
