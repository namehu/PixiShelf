...
发现来源作品行常驻显示“已绑定：名字”“待生效：名字”，两者均无时显示“未绑定艺术家”，名称遵循隐私模式。封面通过受保护目录跳转在新标签页打开外部原站，右侧“站内缩略图预览”保留现有本站浏览能力；全局已忽略仍使用首图弹窗。单条与批量忽略均先确认，弹窗固定本次对象并显示单条标题或实际可忽略数量，说明跨来源持续生效和恢复入口；失败保留弹窗重试，成功保留撤销。此调整不改变绑定、忽略或恢复数据语义，无迁移，回退 App 即可恢复旧交互。
