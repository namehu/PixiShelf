...
E-Hentai 媒体项的后续下载尝试会读取图片页 `loadfail` 入口中的 `nl` 参数，在同一图片页追加该参数请求一次刷新，再解析新媒体地址。只读取有界参数，不执行远端脚本；缺少或非法参数时沿用普通解析。每次尝试最多增加一次受 Provider governor 管理的页面请求，总下载尝试次数、退避、取消与原图/展示质量规则不变。刷新是请求原站重新分配来源，不能保证每次都会分配不同节点。人工重试重置次数后先进行普通下载，仍失败则后续尝试刷新。

媒体下载流同步计算 SHA-1；仅当最终 HTTPS H@H 文件地址具有已识别的 `/h/` 或 `/om/` 文件摘要路径时，与该表示对应的摘要比对，不使用图片页 token 代替文件摘要。`/om/编号/原图描述/派生图描述/...` 使用派生图摘要；第四段为 `x` 时使用原图摘要，无法识别时不猜测摘要。长度正确但摘要不一致的内容仍拒绝保存，并作为可重试媒体损坏处理。正常图片继续执行现有大小、尺寸和首帧元数据校验，不降低图片校验要求。
...
同页的“发现来源”页签默认填写 E-Hentai 完整上传者名称并自动识别账号，数字 UID 收入高级选项。来源不会定时扫描或自行收件；管理员只能手动执行“扫描最新”或在存在历史游标时“继续扫描更早内容”。每次最多持久化 100 条结果，并区分新归档、已有活动任务、已归档且未变化、可能更新和替代版本。只有新归档、可能更新和替代版本可由管理员勾选，选定模式与画质后加入既有收件箱，解析后遵循同一自动或手动入队规则。
...
详情头常显来源身份、关键状态及扫描操作，活动扫描可直接取消；低频来源管理动作集中在更多菜单，扫描覆盖及历史详情可展开查看。需要处理的错误和 UID 提示直接展示。结果工具栏保留范围和列表/首图/卡片切换，入箱模式及画质以摘要显示，点击后在设置弹窗中调整，单项与批量入箱共用当前选项。
...
两类新增来源表单共用名称输入：停止输入一秒后核验完整名称，单轮等待上限二十秒。已保存上传者按名称提供候选；选择已有账号不必再次联网。输入仍按名称解释，包括纯数字名称；手动数字 UID 需显式打开高级选项。

点击保存会等待当前识别，成功后一次保存账号与名称；缺少公开画廊、无身份依据、限流、超时或访问失败时保留名称条件，并明确提示“按名称搜索”。无效查询字符必须修正，不能退化成不限上传者。相同名称在途识别合并，改名或关闭后旧响应失效，超时中止 Provider 请求；识别不创建扫描、收件或下载任务。

新增上传者命中已有来源时直接打开原记录，不自动恢复停用状态，也不合并历史。旧名称来源以中性的“按名称搜索”展示，下次人工扫描继续自动识别；可从更多菜单识别账号，手动纠错仍保留扫描范围复核确认。

标题来源的名称限制与 UID 限制互斥，分别去重；名称条件同时验证标题和上传者元数据，未命中项仍计入候选和续扫进度。条件保存后冻结，未来账号识别不修改原条件，可通过“另存为精确账号筛选”重新识别并另存。展示名称不参与去重键、匹配和游标，复用旧 UID 条件时只补充缺失展示名称，不改变状态、名称、条件或扫描历史。
...
“发现来源”可切换上传者和标题关键词两类。关键词来源保存一个普通文本关键词，匹配方式默认为“包含”，也可选“开头是”或“结尾是”；可选填写完整上传者名称：识别成功保存 UID 条件，暂未识别则保存名称条件；留空才表示不限上传者。名称可改，查询条件只能另存来源。规范化相同的查询复用原记录，不修改原名称或停用状态；停用来源可重新启用。

英/日标题分别忽略大小写、首尾空白后匹配，任一满足即可，保留标点、括号、下划线和内部空白。下划线允许作为标题关键词的一部分，本地按字面匹配，不作为通配符。不运行用户正则。远端只构造受限的标题短语与可选 UID 查询；不能无歧义表达的双引号、星号、百分号及控制字符会明确报错，具体校验以共享 Zod 契约为准。
...
| `BACKGROUND_WRITER` | 其余 27 类任务，包括归档下载                                           | 1        |
...
发现来源（上传者或标题关键词）支持配置最多 200 个固定艺术家／社团。每次创建扫描任务时冻结配置，扫描期间修改配置只影响下一次任务。同一来源的画廊首次匹配时追加这些艺术家：本地已有有效作品立即生效，否则保存为待生效绑定，归档发布成功时在同一事务中应用。绑定不会启动归档或下载。

“首次匹配”由 Catalog 的 `firstMatchedAt` 表示，与全局 NEW 分类无关；不匹配结果变为匹配时才消耗机会。空配置、全局忽略、回收站作品或手动抑制也会消耗首次机会，不会在以后扫描中隐式补绑。迁移把已有匹配结果标记为历史结果，历史补绑需手动选择。

发现结果支持混合状态多选与已加载结果全选，每批最多 100 项。绑定追加并去重，不替换已有作者；忽略与加入收件箱各处理符合条件的子集。“仅看未绑定”在服务端分页前排除已有有效作者或待生效作者的画廊，标签计数同步使用该筛选。

待生效绑定按 provider/gallery/artist 唯一，跨来源共享；来源删除、归档失败或取消不丢失。`discoveryView=pending-creators` 是全局待生效管理入口，每条表示一组画廊／艺术家关系，可批量取消。取消只影响仍待生效的关系，已生效关系在作品管理修改。

手动取消或移除写入独立抑制记录，阻止所有发现来源再次自动追加该画廊的该艺术家；显式手动重新绑定解除抑制。不同 gallery ID 独立，原站 artist/group 标签同步仍遵循原有 SOURCE 证据语义。来源配置变化不追溯修改已经保存的关系。

批量绑定和取消使用持久逐项操作回执。网络结果未知时重用原请求编号，已知失败项重试使用新编号。这两类回执永久保留，避免清理后重放旧请求恢复已经取消的关系。写接口使用管理 procedure、读取使用会话认证，操作者从会话取得，画廊身份由服务端 Catalog 解析。

发现来源作品行常驻显示“已绑定：名字”“待生效：名字”，两者均无时显示“未绑定艺术家”，名称遵循隐私模式。首图模式的封面通过受保护目录跳转在新标签页打开外部原站，右侧“站内缩略图预览”保留现有本站浏览能力；全局已忽略仍使用首图弹窗。单条与批量忽略均先确认，弹窗固定本次对象并显示单条标题或实际可忽略数量，说明跨来源持续生效和恢复入口；失败保留弹窗重试，成功保留撤销。此调整不改变绑定、忽略或恢复数据语义，无迁移，回退 App 即可恢复旧交互。

## 归档任务的艺术家绑定管理

`/admin/archive` 的桌面任务表格、移动卡片与任务详情按 `providerKey + externalId` 展示实际已绑定和待生效艺术家／社团；不把来源默认配置或旧 `Artwork.artistId` 当作当前绑定。列表最多展示 3 位及剩余数量，详情展示完整名单，名称遵循隐私模式。手动链接产生、没有发现来源目录记录的任务也支持此能力。

“仅看未绑定艺术家”要求同时不存在待生效记录和有效作品的生效关系，在数据库分页前执行，并与状态、来源、归档类型、提交批次和搜索条件组合。直接按任务 ID 读取详情不继承列表筛选。

“管理艺术家”支持单条追加和逐项确认移除，不提供任务批量绑定。未归档（包括失败、取消）时追加为待生效，已发布时立即更新当前作品；不会启动下载，也不修改来源默认配置。同一原站身份的其他任务和来源共享结果。回收站作品、非 ACTIVE 作品及正在清理的任务拒绝编辑。移除沿用手动抑制，阻止发现来源自动补回；显式追加可以恢复绑定。

`archive.editTaskCreators` 为管理 mutation，仅接受任务 ID、ADD/REMOVE、最多 200 个艺术家 ID 和 UUID 请求编号；身份与操作者由服务端取得。事务按既有创作者→发布→原站身份锁顺序重读当前发布状态，避免提交期间发布完成导致移除遗漏。操作复用永久保留的 `BIND_CREATORS` 回执，指纹包含 ADD/REMOVE 和规范化的艺术家列表。网络结果未知时保留原编号（含当前浏览器会话恢复），已知失败后重试生成新编号。回放旧追加回执不会恢复后来已移除的绑定。

本次无数据库迁移、文件写入或 Worker 协议变化。回退 App 可恢复旧页面，已保存的关系和抑制记录继续遵循既有发布规则；需要撤销具体关系时使用显式追加／移除，不删除回执或通过重放请求恢复。发布验证使用独立临时 PostgreSQL 空库部署现有迁移，不操作业务数据库或真实媒体。


### 发现结果展示模式

来源结果和全局已忽略共用“纯列表 / 首图预览 / 卡片”偏好，保存在当前浏览器；默认纯列表，旧偏好继续有效。首图使用 112×160px，结果区域达到 576px 时使用 160×208px。卡片上方完整显示封面（高 288px，不裁切），下方显示标题、可用的艺术家绑定、状态、页数和时间；旧记录没有页数时省略，页数读取已有比较快照，不新增扫描。已忽略显示保留的来源与忽略时间。

卡片按结果区域宽度排列：不足 600px 一列，600px 起两列，960px 起三列；移动端一列。发现来源工作区使用后台可用宽度，列数计算不包含后台导航和来源侧栏。卡片按行虚拟滚动，保留触底分页、跨来源位置恢复和最多 100 条选择；切换模式或调整列数按作品身份恢复所在行。

来源卡片图片打开现有站内缩略图预览，另有“打开原站”入口；全局已忽略卡片图片打开封面大图。无首图或加载失败时保留同尺寸占位，来源结果仍可用独立预览入口。图片和敏感摘要沿用隐私模式。无数据库迁移。

### 标题查询校验失败与遗留扫描恢复

Worker 领取标题扫描后，在可统一结算错误的边界内校验冻结查询；校验失败不访问远端、不重试，在同一结算事务中将扫描与后台任务置为失败，并保留错误。旧版本可能留下后台任务 FAILED、扫描 PENDING 的组合。恢复前备份相关扫描记录，按精确任务 ID 锁定关联的两行，仅在任务 FAILED、无租约且扫描 PENDING、检查数为零时，将任务错误与结束时间同步到扫描；保留任务历史，不重置游标、不删除来源或作品。

发布核验应以运行镜像的 revision 为准：当前发布工作流在分支上手动构建只推送 SHA 标签，只有 v 版本标签构建会更新 latest。重启现有容器不会切换已拉取的新镜像。旧失败记录也不会因发布自动恢复。

## 发现来源批量扫描

来源列表支持逐项勾选、全选当前类型筛选和“批量扫描最新”。切换类型保留勾选并提示隐藏选中数；停用和已有活动扫描的来源不可选。启动面板冻结来源顺序，提示包含补全未扫描历史。每次只允许一个未结束批次，暂停中的批次也占用该位置。

后台按固定顺序处理来源：连续 LATEST 直到增量游标耗尽，再连续 HISTORY 直到历史游标耗尽，之后处理下一个来源。首次来源会补完整个历史；历史已完整的来源只追新增。本批不会在补历史后再次回头追扫描期间新发布的内容。NAME 自动绑定 UID 后重新从 LATEST 核对 UID 覆盖。每轮沿用 100 个候选上限、限流、去重、创作者绑定与重试策略；不自动创建收件或下载。

启动和实际扫描均检查来源资格，已删除、停用或被其他扫描占用的来源跳过。单来源最终失败后继续其余来源，终态汇总可仅重试失败及父任务基础设施故障时尚未处理的来源。新批次创建前取消失败父任务遗留的活动子扫描，避免来源永久占用。

暂停阻止后续轮次，已排队子任务直接暂停，运行中的一轮完成后保留游标；若该轮转入重试等待，父暂停状态阻止再次领取。恢复沿检查点继续；取消安全中断当前轮、停止后续派发，保留已发现结果，未提交轮次可能在下次重扫。任务中心对子任务的暂停、继续和取消转交所属批次。

页面的持久批次卡展示来源级已处理/总数、成功/失败/跳过、当前阶段和本轮检查数，支持逐来源结果与详情导航。活动批次每 3 秒刷新，关闭页面不停止任务，重新进入恢复状态。来源名及错误摘要遵循隐私显示与脱敏规则。

批次使用 ARCHIVE_DISCOVERY_BATCH_SCAN@v1 和 SystemJob 父子关系；payload 保存来源顺序，result 保存检查点与控制意图。父任务每次只运行一个短事务，原子创建单轮扫描与保存检查点，通过 preserveAttempt 让出解析通道，避免占道等待子任务。请求 ID 幂等、事务锁和数据库活动批次唯一索引共同防重。扫描历史沿用 30 天清理，活动父批次依赖的扫描记录排除在清理范围外；父任务检查点沿用 SystemJob 保留方式，不依赖短期事件记录。

发布需执行 20260920120000_add_discovery_batch_scan 迁移，扩展任务通道约束并增加活动批次唯一索引，无新增表或数据回填。App 与 Worker 协调升级；回退前取消并等待活动批次及子任务终止，保留扩展约束、索引、来源目录和游标。
