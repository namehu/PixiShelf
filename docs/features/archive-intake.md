...
媒体下载流同步计算 SHA-1；仅当最终 HTTPS H@H 文件地址具有已识别的 `/h/` 或 `/om/` 文件摘要路径时，与该表示对应的摘要比对，不使用图片页 token 代替文件摘要。长度正确但摘要不一致的内容仍拒绝保存，并作为可重试媒体损坏处理。正常图片继续执行现有大小、尺寸和首帧元数据校验，不降低图片校验要求。
