...
收件项目平铺在全局 FIFO 中，submission 只标记一次提交和提供审计/筛选上下文，不形成必须等待全部完成的封闭批次。添加链接和人工勾选发现结果时，按次选择 `AUTO`（解析并归档）或 `MANUAL`（仅解析）以及 `ORIGINAL` 或 `DISPLAY` 画质；界面每次默认 AUTO/ORIGINAL。旧调用缺省模式仍为 MANUAL，数据库历史记录不自动启动。模式和画质参与提交幂等指纹，失败重试与修正链接继承原选择。

AUTO 模式的新作品解析成功直接创建下载任务；已有活动任务则关联并保留其画质；同一作品有变化时留在 `READY` 等待确认；未变化时进入 `SKIPPED`，不重复下载。MANUAL 模式保留解析后选项确认和批量入队。管理员可以在解析继续进行时选择待确认 `READY` 项并调整画质后入队；每项独立返回创建、复用、跳过、冲突或失败结果。原图不可用继续暂停，不自动降低画质；普通网络错误沿用现有重试策略。

解析结果区分新归档、更新、无变化、已有活动任务和重复来源身份。解析快照到期后不能直接入队，必须重新排队解析；修正失败 URL 会创建关联的新项目，不改写原审计历史。

同页的“发现来源”页签支持保存 E-Hentai 上传者名称或数字 UID，UID 是优先的稳定身份。来源不会定时扫描或自行收件；管理员只能手动执行“扫描最新”或在存在历史游标时“继续扫描更早内容”。每次最多持久化 100 条结果，并区分新归档、已有活动任务、已归档且未变化、可能更新和替代版本。只有新归档、可能更新和替代版本可由管理员勾选，选定模式与画质后加入既有收件箱，解析后遵循同一自动或手动入队规则。
...
PC 任务列表使用紧凑表格，移动端使用精简卡片，默认展示标题、状态、来源、图片数量与简短时间。进度仅在运行态展示，失败或需要画质确认时展示处理提示；质量、重试次数、批次、完整时间和已发布媒体预览集中在任务详情。可用本地作品的标题进入作品详情，否则打开任务详情。“原站”入口在新标签页打开该次归档的规范化画廊页。

### 发现来源导航与移动端操作

发现来源在 1024px 及以上使用来源列表与详情的左右分栏；更窄屏幕先显示来源列表，点击来源后显示同一份详情，顶部提供“返回来源列表”。手机详情隐藏重复的工作区标题、页签和来源列表工具区。两端共用查询、操作和业务组件，屏幕宽度只控制布局与滚动方式。桌面自动预选首个来源不会创建详情导航记录，缩窄屏幕时只有显式打开的详情继续显示。

导航仍使用 `/admin/archive/inbox`：`tab=uploaders` 定位发现来源，`sourceId` 定位来源详情，`discoveryView=ignored` 打开与来源详情同级的全局已忽略视图。原有 `itemId` 收件深链保持有效，显式切换工作区时清理另一工作区的定位参数。列表进入详情可通过浏览器返回；详情内换源替换当前详情记录。直接打开详情链接时，页面返回按钮仍可回到来源列表。失效来源提供返回入口，加载错误提供重试，不把请求失败当作来源被删除。

本次页面访问内，从详情返回列表再进入同一来源会保留筛选、未提交勾选、入箱选项和浏览位置；切换不同来源时清空勾选并恢复 AUTO/ORIGINAL。断点切换不清空勾选或增加导航历史；刷新只恢复 URL 定位，不持久化未提交勾选。轮询后已不具备操作资格的项目会从选择中移除。新增上传者、关键词及另存条件后统一进入返回的来源，UID 冲突跳转使用相同入口；删除当前来源后返回列表。

详情头常显来源身份、关键状态及扫描操作，活动扫描可直接取消；低频来源管理动作集中在更多菜单，扫描覆盖及历史详情可展开查看。需要处理的错误和 UID 提示直接展示。结果工具栏保留范围和列表/首图切换，入箱模式及画质以摘要显示，点击后在设置弹窗中调整，单项与批量入箱共用当前选项。

勾选结果后显示底部批量操作条，提供清除、忽略及入箱；全局已忽略提供恢复操作。手机操作条位于应用主导航上方，并为结果内容预留空间。发现结果与全局已忽略共用虚拟列表滚动组件：桌面在固定高度结果区滚动，手机结果随页面滚动，不再形成嵌套纵向滚动。列表每批加载 50 条、最多选择 100 条，恢复浏览位置时以当前数据中的条目和偏移为依据，避免沿用尺寸或数据变化前的测量结果。
...
- `ArchiveIntakeSubmission` 与 `ArchiveIntakeItem`：保存输入、FIFO 顺序、解析状态、冻结结果、下载模式、画质、重试/取消 intent 和归档关联；
...
自动入队在 resolver 的 fenced 终态事务中调用 App 手动入队共用的核心实现，原子提交解析结果、下载任务与事件。发布锁下重新判断已有归档与活动任务，避免并发发布后误将新建变成自动更新；媒体下载继续由 writer 执行。`SKIPPED` 属于终态，归入取消/跳过历史及既有 30 天保留清理。

解析、发现来源和下载共享 Provider 请求间隔与真实限流冷却；普通请求默认间隔 250ms，搜索请求获准后设置 3 秒间隔。活跃下载许可只限制下载并发，不再使解析退队。因此前一作品下载时，后续作品可以继续完整解析并自动入队，无需等待媒体流结束。真实站点限流仍按既有策略重试；共享请求机会不保证严格轮转或防饥饿，下载请求可能因解析请求而稍后获准。下载流的许可仍持续到流结束，并保持续期与过期回收。
...
生产 capability inventory 固定为 30 个 job type；`SCAN` 支持 v1/v2/v3，`ARCHIVE_IMPORT` 支持 v1/v2，其余 28 类只支持 v1，共 33 个
...
`20260909100000_archive_intake_download_mode` 新增下载模式和 `SKIPPED` 终态，历史模式默认 MANUAL，画质仍默认 ORIGINAL。正式升级按[备份与恢复](../operations/backup-and-recovery.md)建立一致性检查点，以 generate/deploy 流程部署迁移并同步升级 App/Worker。开发验证使用隔离库，不迁移现有收藏库。

回滚前停止新提交并等待或取消活动任务；保留新增字段与已创建任务，不通过降级 SQL 删除记录或媒体。旧 Worker 不执行自动衔接，旧 App 不理解 SKIPPED，不能混合版本继续处理自动模式记录；可恢复同一版本，或在停写后成组恢复数据库与媒体检查点。
