...
v1/v2/v3，`ARCHIVE_IMPORT` 注册 v1/v2，其余 28 类只注册 v1，共 33 个 job type/definition-version 组合。capability audit 精确验证 type、
...
4. 生产 capability inventory 固定为 30 个 job type、33 个 type/version 组合；`SCAN` 支持 v1/v2/v3，
   `ARCHIVE_IMPORT` 支持 v1/v2，其余 28 类只支持 v1，任务类型、definition version 与 lane 必须精确匹配。
