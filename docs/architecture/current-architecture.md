...
两个 lane 可以各运行一个任务，同一 lane 内不能并行。生产 Registry 保持 31 个 job type：`SCAN` 同时注册
v1/v2/v3，`ARCHIVE_IMPORT` 注册 v1/v2，`ARCHIVE_SEARCH_SCAN` 注册 v1/v2/v3，其余 28 类只注册 v1，共 35 个 job type/definition-version 组合。capability audit 精确验证 type、
...
4. 生产 capability inventory 固定为 31 个 job type、36 个 type/version 组合；`SCAN` 支持 v1/v2/v3，
   `ARCHIVE_IMPORT` 支持 v1/v2，`ARCHIVE_SEARCH_SCAN` 支持 v1/v2/v3，其余 28 类只支持 v1，任务类型、definition version 与 lane 必须精确匹配。
