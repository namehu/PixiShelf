...
本地目录导入由预览和执行两个显式阶段组成。预览阶段遍历 `local-imports`、识别包含直属媒体的作品目录，并用数据库路径标记新增与已有作品；开始导入时，浏览器只提交该次预览中的新增作品路径。App 校验路径、再次按精确路径去重，并把作品路径、艺术家映射和默认标签冻结到任务快照后立即入队。Worker 只读取快照列出的作品目录，不重新遍历 `local-imports` 根目录，也不计算媒体内容指纹。单作品重扫是独立链路，仍使用内容指纹检测扫描与执行之间的源文件变化。
