...
| 扫描调用方            | `Authorization: Bearer <SCAN_WEBHOOK_TOKEN>` | `/api/webhooks/scan` 的健康检查和扫描触发              | 该 Token 不能调用 scheduler 或浏览器会话接口 |
...
| `GET/HEAD/POST /api/webhooks/scan`              | 公共 transport + `SCAN_WEBHOOK_TOKEN`；未配置返回 `503` | 健康检查或入队/执行扫描                       | 高；Token 泄露可触发高成本扫描与数据库写入                      |
...
| `exportIncompleteArtistsAction`                          | `authActionClient`                        | 读取并导出不完整 Artist 标识                            |
