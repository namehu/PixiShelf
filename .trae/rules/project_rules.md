...
当你需要验证启动服务时，需要执行以下步骤：

1. 检查服务是否启动并且正常运行。后端监听3001端口，前端监听3000端口。
2. 后端服务如果已经启动。可以通知用户手动重启，并且等待5s后自行执行下一步即可
3. 如果服务没有启动。那么你可以自己进行启动

## 6A工作流执行规则

用户显式的输入 6A工作流/6A 等指令，触发 6A 工作流执行。否则按正常逻辑执行。
